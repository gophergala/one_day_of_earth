...
func SearchImages(lat, lng string, minTimeStamp, distance int) (images []APIs.ApiImage, cerr *lib.CError) {
	...
	data, cerr := APIs.API_call(config.GET, "https://www.flickr.com/services/rest/", map[string]string{
		...
		"lat":             lat,
		"lon":             lng,
		"min_upload_date": strconv.Itoa(minTimeStamp),
		"max_upload_date": strconv.Itoa(minTimeStamp + 86000), // Flickr not filtering by time if there only min time
		"radius":          strconv.Itoa(distance),
		"format":          "json",
		"nojsoncallback":  "1",
		"per_page":        "500",
	})
